...
- **정확도 검증**: 사람이 실제로 검토한 81건(`문화정책_라벨링_중앙·서울_코드기준.xlsx`, 중앙부처+서울 복지문화 계열)을 정답으로 씁니다. 이 중 12건을 프롬프트에 few-shot 예시로 보여주고, 나머지 69건으로 재는 held-out 정확도가 진짜 실력이고 현재 73.9%(51/69)로 기존 baseline과 거의 동률입니다. 81건 전체(few-shot 포함, 데모에서 보여주는 숫자)는 77.8%(63/81)인데 — few-shot 12건은 정답을 이미 프롬프트에 보여준 예시라 12/12 트리비얼하게 맞으므로 이 숫자를 실제 정확도로 오인하면 안 됩니다. "확인필요"라는 애매한 카테고리가 생기면서 판정 난이도가 올라, 있음/없음/비율기반 3지선다였을 때의 81%보다는 낮아졌고, 소득공제·본인부담금 차액 지원 같은 규칙을 추가해도 다른 케이스에서 상쇄돼 순변화가 거의 없었습니다. 81건이라는 표본 자체가 작아서 1건이 ±1.4%p를 움직입니다.
- 이 판정을 온통청년 전체 정책 2,646건에 돌려서 Supabase `policy_support_amounts` 테이블에 적재합니다 (`BE/AI/backfill_policy_support_amounts.py`) — 있음(고정금액) 1,090건, 확인필요 102건, 없음(금액 없음) 1,454건.
...
- 자립준비청년·가족돌봄청년·한부모·고립은둔청년·출산·빈곤(근로인센티브 등)처럼 개인 사정에 따라 대상·금액이 달라지는 정책은 confidence를 의도적으로 낮게 매기도록 프롬프트에 명시했습니다 — "가족돌봄청년 전담지원 시범사업"이 원문만 보면 그럴듯한데 실제로는 오답인 채로 confidence 0.95를 받았던 사례를 발견해서 고쳤습니다. 다만 "이름에 이 단어들이 들어가면 무조건 확인필요로 바꿔라" 같은 이름 기반 강제 매핑은 시도해보고 기각했습니다 — 81건 시뮬레이션 결과 정확도가 77.8%→64.2%로 오히려 크게 떨어졌습니다(이미 맞던 답 11건이 틀어지고, 틀리던 답 1건만 고쳐짐). 같은 이름(예: "청년내일저축계좌")이어도 인스턴스마다 정답이 다른 경우가 많아, 이름만으로 일괄 처리하기보다 LLM이 원문을 보고 건별로 판단하게 두는 쪽이 더 정확했습니다.
...
- 지원금액 판정 정확도 개선 — 정답 검증이 81건(그중 12건은 few-shot으로 이미 노출)뿐이라 표본이 작고, held-out 정확도(73.9%)가 기존 baseline과 거의 동률에 그침. 더 큰 사람 검토 표본을 확보하거나 "확인필요" 오분류(있음/없음인데 확인필요로 도망가는 경우) 패턴을 더 다듬을 여지가 있음
