...
앱/웹에 실제로 보이는 이름은 전부 **"모아폴리"**로 통일했습니다 (웹 탭 제목, iOS/Android 홈 화면 앱 이름 등 — `web/index.html`, `web/manifest.json`, `ios/Runner/Info.plist`, `android/app/src/main/AndroidManifest.xml`). "AICPP"는 GitHub 저장소 이름과 Dart 패키지 이름(`pubspec.yaml`의 `name:`, 모든 `import 'package:aicpp/...'`)으로만 남아있습니다 — 코드 전체에 퍼져 있는 내부 식별자라 바꾸는 실익이 없어 그대로 뒀습니다. 배포 도메인(`aicpp.onrender.com`, `aicpp-mu.vercel.app`)과 iOS 번들 ID(`com.aicpp.aicpp`)도 실제 서비스 재설정이 필요한 인프라 값이라 그대로입니다. 앱 전체 색도 원래 토스 블루였던 기본 강조색(`TossColors.primary`)을 모아폴리 개구리 초록색으로 바꿔서, 버튼/선택 상태/탭 표시 등 앱 전반이 하나의 브랜드 색으로 통일됩니다 (카카오톡/구글/Apple 소셜 로그인 버튼은 각 사 고유 브랜드 색이라 그대로 둠).
...
- **지도** (`lib/screens/main_screen.dart`)
  ...
  - 네이버 지도는 Android/iOS 네이티브 SDK 기반이라 웹에서는 지원되지 않습니다. 그래서 **웹 빌드는 실제 지도 대신 "연잎 지도"라는 자체 일러스트 시각화(`lib/widgets/web_region_map.dart`)를 씁니다** — `kIsWeb`으로 분기해서 네이티브 코드는 그대로 둡니다. 17개 시도 전부를 한 번에 연잎(원) 모양으로 보여주고, 정책 건수가 많을수록 잎이 커지며, 내 지역은 앱 아이콘과 같은 진한 초록 + 개구리 마스코트가 위에 앉은 모습, 관심지역은 중간 톤 초록, 나머지는 연한 초록으로 구분합니다. 실제 위경도 기반 지도가 아니라 고정된 상대 위치에 배치한 일러스트라 확대/축소는 없습니다 (네이티브만 실제 지도로 확대/축소·팬 가능). 이전엔 `flutter_map` + OpenStreetMap 타일로 실제 지도를 웹에도 띄웠었는데, 자기 지역/관심지역 3곳만 덩그러니 떠 있는 실제 지도보다 17개 지역을 한눈에 보여주는 이 일러스트가 더 어울린다고 판단해 교체했습니다 — `flutter_map`/`latlong2` 의존성과 옛 위경도 데이터(`lib/constants/region_latlng.dart`)는 되돌릴 수도 있어 지우지 않고 주석 처리만 해뒀습니다. 화면 제목도 "관심지역 지도"에서 **"폴리 지도"**로 바꿨고, 그 아래에 네이티브와 같은 형태의 범례("내 지역"/"관심지역" 색 설명, `_WebMapLegend`)를 웹 색상에 맞춰 따로 보여줍니다. macOS 데스크톱 타깃은 이 프로젝트에 구성돼 있지 않습니다.
...
- **리포트** (`lib/screens/report_screen.dart`)
  - 거주지역 + 관심지역의 정책을 모아 매칭 자격 충족률(도넛 게이지, 나이·소득 조건까지 반영해 자격 되는 건수 ÷ 전체 건수), 카테고리 분포(도넛 차트, 자격 매칭 여부와 무관하게 **사용자가 직접 스크랩한 정책만으로 계산** — `categoryLabel`로 정규화된 값을 쓰기 때문에 같은 카테고리의 다른 표기가 따로 집계되지 않고, 위 지도 탭과 같은 로직이라 복지/문화도 여기서 자동으로 나뉘어 집계됩니다. 카테고리별 색도 건수 정렬 순서가 아니라 카테고리 이름에 고정 매핑해서, 스크랩 구성이 바뀌어도 같은 카테고리는 항상 같은 색으로 보입니다), 마감임박 정책 타임라인(D-day 배지)을 보여줌
...
- 버튼 아이콘은 앱 아이콘/스플래시와 같은 개구리 캐릭터(텍스트만 뺀 버전, `assets/icon/assistant_icon.png`)이고, 버튼·전송 버튼·내 말풍선 색도 앱 아이콘의 개구리 초록색을 씁니다. 패널 제목도 "모아폴리 정책 어시스턴트"로 통일했습니다. (이 초록색은 이제 `TossColors.primary`, 즉 앱 전체의 기본 강조색입니다 — 아래 "디자인 통일" 참고.)
...
- 말투는 반말로 고정합니다 (원래는 존댓말이었는데, "폴리"라는 친근한 개구리 캐릭터 톤에 맞게 반말로 바꿨습니다). 바로 신청 가능하다고 확신을 갖고 알려주는 문장 끝에는 🐸 이모지를 붙이고("서울 사는 21살이면 이 3건은 바로 신청 가능해! 🐸"), 나이·소득분위처럼 사용자 정보가 미상이라 조건 충족 여부를 확신할 수 없는 정책은 무엇을 모르는지 먼저 밝히고 말끝을 흐리며 확인이 필요하다고 말하도록(예: "소득 구간을 아직 몰라서, 임차보증금 지원은 확인이 필요해..") 시스템 프롬프트에 명시했습니다 (`BE/app/services/chat_service.py`).
...
관객/심사위원이 각자 휴대폰으로 접속해서 써보는 데모처럼 설치 없이 여러 사람에게 빠르게 보여줘야 할 때 이 방식을 씁니다 (반대로 네이티브 앱 배포는 iOS는 유료 개발자 계정 + 심사 대기, Android도 APK 설치 마찰이 있어 더 느립니다). 지도만 위에서 설명한 대로 연잎 지도 일러스트로 바뀌고, 나머지 기능(정책 조회/필터, 리포트, 폴리 채팅, 프로필)은 네이티브와 동일하게 동작합니다.
...
- 네이버 지도 플러그인(`flutter_naver_map`)이 Android/iOS만 지원하므로, **네이버 지도 자체까지 확인하려면 iOS 시뮬레이터(또는 Android 에뮬레이터/실기기)로 실행**해야 합니다. 웹은 지도가 연잎 지도 일러스트(`WebRegionMap`)로 자동 대체되고, 나머지 기능은 네이티브와 동일하게 동작합니다.
...
- **웹은 실제로 배포되어 있습니다** (https://aicpp-mu.vercel.app , 백엔드 https://aicpp.onrender.com) — 지도만 연잎 지도 일러스트로 대체되고 나머지 기능은 네이티브와 동일하게 동작합니다. 둘 다 무료 플랜이라 트래픽이 몰리면(대략 동시 30명 이상) 정부 API·NewsAPI 호출 제한이나 Render 무료 인스턴스의 단일 프로세스 한계로 느려질 수 있습니다.
...
- `flutter analyze`, `flutter test`(94개), 백엔드 `unittest`(46개) 모두 통과하는 상태입니다.
