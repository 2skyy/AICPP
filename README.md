...
- **지도** (`lib/screens/main_screen.dart`)
  ...
  - 네이버 지도는 Android/iOS 네이티브 SDK 기반이라 웹에서는 지원되지 않습니다. 그래서 **웹 빌드는 실제 지도 대신 "연잎 지도"라는 자체 일러스트 시각화(`lib/widgets/web_region_map.dart`)를 씁니다** — `kIsWeb`으로 분기해서 네이티브 코드는 그대로 둡니다. 17개 시도 전부를 한 번에 연잎(원) 모양으로 보여주고, 정책 건수가 많을수록 잎이 커지며, 내 지역은 앱 아이콘과 같은 진한 초록 + 개구리 마스코트가 위에 앉은 모습, 관심지역은 중간 톤 초록, 나머지는 연한 초록으로 구분합니다. 실제 위경도 기반 지도가 아니라 고정된 상대 위치에 배치한 일러스트라 확대/축소는 없습니다 (네이티브만 실제 지도로 확대/축소·팬 가능). 이전엔 `flutter_map` + OpenStreetMap 타일로 실제 지도를 웹에도 띄웠었는데, 자기 지역/관심지역 3곳만 덩그러니 떠 있는 실제 지도보다 17개 지역을 한눈에 보여주는 이 일러스트가 더 어울린다고 판단해 교체했습니다 — `flutter_map`/`latlong2` 의존성과 옛 위경도 데이터(`lib/constants/region_latlng.dart`)는 되돌릴 수도 있어 지우지 않고 주석 처리만 해뒀습니다. macOS 데스크톱 타깃은 이 프로젝트에 구성돼 있지 않습니다.
