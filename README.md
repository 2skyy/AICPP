...
- **채팅** (`lib/screens/chat_screen.dart`) — 정책 어시스턴트
  - 빈 상태에서는 추천 질문 칩 노출, 탭하면 바로 전송
  - 사용자/어시스턴트 말풍선 구조이며, 어시스턴트 응답에는 "컨텍스트: 지역 · 재학상태 · 나이" 태그가 붙음
  - `lib/utils/chat_context.dart`에서 프로필 기반 컨텍스트 라벨과, 추후 LLM에 넘길 시스템 프롬프트(`buildSystemPrompt`)를 준비해뒀지만, 아직 LLM 연동 전이라 실제 답변은 플레이스홀더 텍스트로 대체됨
...
- 채팅 탭은 UI와 프로필 기반 컨텍스트/시스템 프롬프트 준비까지만 되어 있고, 실제 LLM 연동은 아직 없어 답변은 플레이스홀더입니다.
...
- `flutter analyze`, `flutter test` 모두 통과하는 상태입니다 (11개 위젯 테스트).

### 다음에 이어서 할 만한 것
- FastAPI(또는 다른) 백엔드 연동 — 회원가입/로그인/프로필 저장, 실제 이메일 인증
- 카카오톡/구글/iCloud 소셜 로그인 실제 연동
- 채팅에 실제 LLM 연동 (정책 매칭용 실데이터/정책 DB 필요) — `buildSystemPrompt` + 사용자 질문을 API로 전달하고 플레이스홀더 응답 교체
