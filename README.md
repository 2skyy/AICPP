...
- **정확도 검증**: 사람이 실제로 검토한 81건(`문화정책_라벨링_중앙·서울_코드기준.xlsx`, 중앙부처+서울 복지문화 계열)을 정답으로 씁니다. 이 중 12건을 프롬프트에 few-shot 예시로 보여주고, 나머지 69건으로 held-out 정확도를 재는데 현재 72.5%로 기존 정규식 기반 판정과 동률입니다 — "확인필요"라는 애매한 카테고리가 늘면서 판정 난이도가 올라, 있음/없음/비율기반 3지선다였을 때의 81%보다는 낮아졌습니다. few-shot 12건 밖의 새 정책에도 규칙이 그대로 통할지는 검증되지 않았고, 81건이라는 표본 자체도 작아서 1건이 ±1.4%p를 움직입니다.
- 이 판정을 온통청년 전체 정책 2,646건에 돌려서 Supabase `policy_support_amounts` 테이블에 적재합니다 (`BE/AI/backfill_policy_support_amounts.py`) — 있음(고정금액) 1,093건, 확인필요 96건, 없음(금액 없음) 1,457건.
- **사람 검토 여부로 신뢰도를 둘로 나눕니다.** 81건 중 사람이 직접 확정한 75건(`BE/AI/apply_human_verified_labels.py`)은 `verification_status = 'VERIFIED'`로 표시해 "확정"으로, 그 밖에 LLM 자체 confidence가 0.9 이상인 값은 "AI 추정(검토 전)"으로, 0.9 미만은 화면·챗봇 어디에도 노출하지 않습니다 (`BE/app/services/policy_amount_service.py`) — 검증 안 된 숫자를 확정치처럼 보여주는 위험을 피하기 위함입니다.
- 자립준비청년·가족돌봄청년·한부모·고립은둔청년처럼 개인 사정에 따라 대상·금액이 달라지는 정책은 confidence를 의도적으로 낮게 매기도록 프롬프트에 명시했습니다 — "가족돌봄청년 전담지원 시범사업"이 원문만 보면 그럴듯한데 실제로는 오답인 채로 confidence 0.95를 받았던 사례를 발견해서 고쳤습니다.
...
- 지원금액 판정 정확도 개선 — 정답 검증이 81건(그중 12건은 few-shot으로 이미 노출)뿐이라 표본이 작고, held-out 정확도(72.5%)가 기존 baseline과 동률에 그침. 더 큰 사람 검토 표본을 확보하거나 "확인필요" 오분류(있음/없음인데 확인필요로 도망가는 경우) 패턴을 더 다듬을 여지가 있음
