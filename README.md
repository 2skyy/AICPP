...
- 정규식/LLM 추출은 "청년에게 실제로 현금이 지급되는가"까지는 구분하지 못해서, 대출 한도·사업 전체 예산·자격조건에 등장하는 숫자를 지원금액으로 잘못 채운 사례가 있었습니다. 복지문화/금융복지문화 대분류(중앙부처+서울, 81건)를 사람이 직접 검토해 만든 라벨링 자료와 대조해서, 사람이 "현금 지급 아님"으로 확정 판정했는데 금액이 들어가 있던 9건을 `금액정보없음`으로 정정했습니다(`sprt_amt_source = '사람검토(문화라벨링)'`로 표시). 나머지(정확한 금액/비율을 몰라 못 채운 건, "확인필요"로 애매한 건)는 추가 정보가 있어야 반영할 수 있어 남겨뒀습니다.
