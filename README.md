...
- 버튼 아이콘은 앱 아이콘/스플래시와 같은 개구리 캐릭터(텍스트만 뺀 버전, `assets/icon/assistant_icon.png`)이고, 버튼·전송 버튼·내 말풍선 색도 앱 아이콘의 개구리 초록색(`TossColors.assistantPrimary`, 나머지 화면은 계속 토스 블루)으로 맞췄습니다. 패널 제목도 "모아폴리 정책 어시스턴트"로 통일했습니다.
