...
- **지도** (`lib/screens/main_screen.dart`)
  ...
  - 핀(웹은 지도 마커)을 탭하면 해당 지역 정책 목록 바텀시트(`lib/widgets/policy_list_sheet.dart`)가 열립니다
    ...
    - "내 관심사만"을 켜면 프로필 관심사와 겹치는 정책에 "관심사: 주거" 같은 뱃지가 카드에 뜹니다 (`PolicyItem.matchingInterests`) — 안 켜져 있을 땐 뱃지도 안 뜹니다. "내 관심사만"과 개별 카테고리 칩은 둘 다 카테고리로 좁히는 같은 목적이라 동시에 켜두면 결과가 헷갈려서, "내 관심사만"이 켜져 있는 동안은 "전체"·개별 카테고리 칩이 흐리게 비활성화되고(선택도 초기화) 다시 끌 때까지 못 누릅니다
...
- **리포트** (`lib/screens/report_screen.dart`)
  - 거주지역 + 관심지역의 정책을 모아 매칭 자격 충족률(도넛 게이지, 나이·소득 조건까지 반영해 자격 되는 건수 ÷ 전체 건수), 카테고리 분포(도넛 차트, 나이·소득 조건까지 반영해 자격 되는 정책 중 **사용자가 프로필에서 관심사로 고른 카테고리만** 집계 — 관심사에 없는 카테고리는 그 정책이 자격 되더라도 도넛에 안 나타나고, 관심사를 프로필 수정에서 추가·삭제하면 도넛도 그대로 따라 바뀝니다. `categoryLabel`로 정규화된 값을 쓰기 때문에 같은 카테고리의 다른 표기가 따로 집계되지 않고, 위 지도 탭과 같은 로직이라 복지/문화도 여기서 자동으로 나뉘어 집계됩니다. 카테고리별 색도 건수 정렬 순서가 아니라 카테고리 이름에 고정 매핑해서 항상 같은 카테고리는 같은 색으로 보입니다), 마감임박 정책 타임라인(D-day 배지)을 보여줌
...
- **`plcyKywdNm`은 자유 텍스트가 아니라 정부가 정한 고정 태그 집합입니다.** 처음엔 `BE/app/services/keyword_extractor.py`가 "일자리"/"문화"/"저축" 같은 일상 단어를 그대로 `topic`에 보내고 있었는데, 실제 API 응답을 대량 샘플링해보니 진짜 태그는 `교육지원`/`보조금`/`벤처`/`중소기업`/`주거지원`/`인턴`/`맞춤형상담서비스`/`장기미취업청년`/`바우처`/`육아`/`출산`/`대출`/`해외진출`/`금리혜택`/`신용회복`/`청년가장`/`공공임대주택` 등 소수의 값뿐이었습니다 — 그래서 "일자리"로 검색하면 에러 없이 성공 응답에 `totCount: 0`이 오는 식으로, 키워드가 인식되는 질문 대부분이 조용히 실패하고 있었습니다. 확실히 대응되는 단어만 실제 태그로 매핑하도록 다시 작성했고, 애매한 것(문화/여가/복지/지원금/생활비/참여/권리 등)은 잘못 매핑하느니 그냥 빼서 아래 지역 기반 폴백으로 넘어가게 했습니다.
...
- 응답을 반환하기 전에 아래 "지원금액 판정 파이프라인 연동" 섹션의 Supabase 데이터를 `plcyNo` 기준으로 병합합니다.

### 지원금액 판정 파이프라인 연동 (Supabase, `BE/AI/`)
- 온통청년 정책 원문에서 실제로 청년 개인에게 현금이 지급되는지를 판정하는 배치를 이 저장소 안에 직접 구축했습니다. 정규식으로 원문의 숫자 후보를 먼저 뽑고, 그 후보와 원문을 Claude(claude-sonnet-5)에게 같이 주고 구조화된 도구 호출(`SupportAmountExtraction` 스키마, `BE/app/schemas/support_amount_extraction.py`)로 있음/없음/확인필요 중 하나를 판정시킵니다 (`BE/AI/support_amount_extraction.py`). 처음엔 비율기반을 별도 카테고리로 뒀는데, 소득구간별 매칭 적립처럼 조건마다 금액 자체가 달라지는 걸 "정확한 하나의 숫자"로 강제하는 게 더 위험하다고 판단해 확인필요로 합쳤습니다.
- **정확도 검증**: 사람이 실제로 검토한 81건(`문화정책_라벨링_중앙·서울_코드기준.xlsx`, 중앙부처+서울 복지문화 계열)을 정답으로 씁니다. 이 중 12건을 프롬프트에 few-shot 예시로 보여주고, 나머지 69건으로 재는 held-out 정확도가 78.3%(54/69)입니다. 81건 전체(few-shot 포함, 데모/보고용 수치)는 80.2%(65/81)인데 — few-shot 12건은 정답을 이미 프롬프트에 보여준 예시라 그만큼 부풀려진 숫자라는 점은 감안해야 합니다. 라벨링 xlsx의 baseline 비교 컬럼도 `베이스라인자동판정`으로 바뀌었는데 이 기준으로는 49.3%(34/69)라 격차가 꽤 큽니다.
- 이 판정을 온통청년 전체 정책 2,646건에 돌려서 Supabase `policy_support_amounts` 테이블에 적재합니다 (`BE/AI/backfill_policy_support_amounts.py`) — 있음(고정금액) 1,030건, 확인필요 176건, 없음(금액 없음) 1,440건.
- **사람 검토 여부로 신뢰도를 둘로 나눕니다.** 81건 전부(`BE/AI/apply_human_verified_labels.py`)는 `verification_status = 'VERIFIED'`로 표시해 "확정"으로, 그 밖에 LLM 자체 confidence가 0.9 이상인 값은 "AI 추정(검토 전)"으로, 0.9 미만은 화면·챗봇 어디에도 노출하지 않습니다 (`BE/app/services/policy_amount_service.py`) — 검증 안 된 숫자를 확정치처럼 보여주는 위험을 피하기 위함입니다.
- **`BE/AI/backfill_policy_support_amounts.py`의 upsert가 한때 VERIFIED(사람 검토) 행까지 덮어쓰는 버그가 있었습니다** — 자립/가족돌봄/한부모 등 이름 패턴으로 재추출을 돌릴 때 사람이 확정한 값이 최신 LLM 추측으로 조용히 바뀌어버렸습니다. SQL에 `WHERE verification_status IS DISTINCT FROM 'VERIFIED'` 조건을 추가해 이제 VERIFIED 행은 재추출 배치가 절대 못 건드리고, 81건 전체를 사람 정답과 대조해 불일치 0건을 확인했습니다.
- **이름 기반 강제 매핑**(`apply_name_based_override`): 정책명에 "자립"/"청년내일저축계좌"/"청년도약"/"한부모"/"빈곤"/"출산"이 들어가면 LLM 판정과 무관하게 확인필요로 강제합니다. 처음엔 이런 블랭킷 매핑을 시도했다가 기각했었는데("자립"만 있어도 81건 시뮬레이션에서 정확도가 77.8%→64.2%로 떨어졌음), 이후 라벨링 담당자가 xlsx의 정답 자체를 "자립" 계열 8건 등에 대해 확인필요로 재확정하면서 판단이 뒤집혔습니다 — 같은 매핑이 이제는 held-out 정확도를 69.6%→78.3%로 올립니다. 다만 이름만으로 판단하면 예외가 생겨서(`NAME_BASED_OVERRIDE_EXCEPTIONS`), "위기청년 자립지원"(정답 있음)과 "고용보험 미적용자 출산급여 지원"(정답 없음)은 매핑에서 뺐습니다. 이 매핑은 원문에 금액이 명확한 정책(예: "서산시 출산지원금" — 첫째/둘째/셋째 자녀별로 정액이 다 나와 있음)도 "출산"이라는 이유만으로 확인필요로 만들어버리는 부작용이 있는데, 라벨링 표본 밖이라 정확도에는 안 잡히는 알려진 트레이드오프입니다.
- 자립준비청년·가족돌봄청년·한부모·고립은둔청년·출산·빈곤(근로인센티브 등)처럼 개인 사정에 따라 대상·금액이 달라지는 정책은 (이름 매핑과 별도로) confidence를 의도적으로 낮게 매기도록 프롬프트에 명시했습니다 — "가족돌봄청년 전담지원 시범사업"이 원문만 보면 그럴듯한데 실제로는 오답인 채로 confidence 0.95를 받았던 사례를 발견해서 고쳤습니다.
- `BE/app/services/ontong_policy_service.py`가 이 결과를 `plcyNo` 기준으로 실시간 API 응답에 병합하고, `BE/app/services/chat_service.py`가 "확정 지원금액" / "AI 추정(검토 전) 지원금액" / "확인필요"를 서로 다른 문구로 폴리 답변에 반영합니다 (숫자를 지어내지 않고 "조건에 따라 달라져서 확인이 필요해"처럼 답하도록 지시).
- **하이브리드 구조**: 정책 검색 자체는 그대로 온통청년 실시간 API를 쓰고, Supabase는 그 위에 지원금액만 얹습니다. Supabase 연결이 실패하거나 해당 정책이 아직 파이프라인에 없어도 예외 없이 빈 값으로 넘어가서, 실시간 정책 조회 자체는 항상 정상 동작합니다. 프론트엔드는 이 값이 없는 정책은 지원금액을 아예 표시하지 않습니다 (위 "지도" 섹션 참고).
...
- 질문 + 프로필(지역/관심지역/재학상태/나이/성별/학교/학점/소득분위/스크랩한 정책)을 받아서: (1) `BE/app/services/keyword_extractor.py`의 주제 키워드 사전으로 질문에서 키워드를 찾아 `topic` 검색(최대 10건), 결과가 비면 거주지역으로 한 번 더 검색, 키워드가 아예 없으면 바로 거주지역 검색 → (2) 검색된 정책(오늘 기준 신청 가능한 것만) + 스크랩한 정책 + 프로필 전체를 Claude에게 근거로 주고 "이 안에서만 답하라"고 지시 → (3) 생성된 답변을 마크다운 제거 후 반환 (`BE/app/services/chat_service.py`). 검색 결과가 10건보다 많으면("총 OO건 중 상위 10건") 폴리가 답변에 전체 건수를 밝히고 더 보고 싶으면 말해달라고 안내합니다.
- 검색된 정책의 URL을 Claude가 직접 만들지 않고, 실제 검색 결과에서 그대로 매핑해서 반환하므로 잘못된 링크가 생기지 않습니다.
- 검색된 정책은 `aplyUrlAddr`(없으면 `refUrlAddr1`), 스크랩한 정책은 프론트가 같은 방식으로 골라 보낸 `apply_url`을 시스템 프롬프트에 "신청링크"로 같이 넣어줍니다 — 원래 이 필드가 빠져있어서, 정책 상세 화면엔 "신청 페이지로 이동" 버튼이 있는데도 폴리는 링크를 모른다고 답하는 불일치가 있었습니다.
- 단순 조회형 작업이라 Claude의 확장 사고(thinking)를 명시적으로 꺼서 응답 속도를 개선했습니다 (아래 "성능 개선" 참고).
- 백엔드는 대화 이력을 전혀 들고 있지 않아서(매 요청이 단일 질문만 Claude에게 보냄), 폴리가 매번 자기소개하며 인사하는 걸 막으려면 프론트엔드가 힌트를 줘야 합니다. 그래서 요청에 `is_first_message`(대화창을 새로 시작한 뒤 첫 질문인지)를 같이 보내고, 시스템 프롬프트가 이 값에 따라 "인사해도 좋다" / "이미 대화 중이니 인사하지 말고 바로 답변부터 시작해"를 갈라 지시합니다 (`lib/widgets/chat_panel.dart`의 `_messages.isEmpty`로 판단, `BE/app/services/chat_service.py`).
- 대화 이력이 없다 보니, 이번 질문에서 검색된 [정책 목록]이 비었을 때 "정책 정보 자체가 없다"는 식으로 과장해서 답하고, 다음 질문에서 목록이 채워지면 "아, 그 말이 아니라 사실 있어"처럼 스스로 답을 뒤집는 문제가 있었습니다. 시스템 프롬프트에 "이번 검색으로는 못 찾았어"처럼 이번 질문에 한정해서 말하고 이전 답을 정정하는 표현은 쓰지 말라고 명시해서 고쳤습니다. 같은 김에 "아는 건 지원금액·신청기간·신청방법까지 구체적으로, 모르는 건 짧고 솔직하게 모른다고" 답하도록도 명시했습니다.
...
- 지원금액 판정 정확도 개선 — 정답 검증이 81건(그중 12건은 few-shot으로 이미 노출)뿐이라 표본이 작음(held-out 정확도 78.3%). 이름 기반 매핑(자립/청년내일저축계좌/청년도약/한부모/빈곤/출산)이 "서산시 출산지원금"처럼 금액이 명확한 정책까지 확인필요로 덮어버리는 부작용이 있어, 더 큰 사람 검토 표본으로 이 매핑의 예외를 더 찾아낼 여지가 있음
- `plcyKywdNm` 실제 태그 매핑표를 이번에 발견한 17개보다 더 넓게 확보 — 지금은 대량 샘플링으로 발견한 것만 매핑했고, 정부 API 공식 문서/코드표를 구할 수 있으면 더 정확하고 빠짐없이 매핑 가능
