"""시도명 -> 온통청년 API `zipCd`(시군구 행정표준코드) 목록 매핑.

코드는 손으로 입력한 게 아니라, 실제 온통청년 API의 전국 단위 정책
("청년주택드림청약통장")이 응답한 zipCd 필드(254개 코드)를 그대로 가져와서
2자리 시도 접두사(11=서울, 41=경기 등 행정표준코드 체계)로 묶은 것이다.

참고: 이 API의 데이터셋에서는 광주광역시와 전라남도가 "전남광주통합특별시"
라는 단일 행정구역(zipCd 접두사 12)으로 합쳐져 있다. 우리 앱은 두 지역을
별도로 두고 있어서, 정렬된 27개 코드 중 앞 5개(광주광역시의 5개 구 순서와
일치)를 광주광역시에, 나머지 22개(전라남도의 시·군 수와 일치)를 전라남도에
배정했다 — 정확한 1:1 매핑이 아니라 추정치이니 참고할 것.
"""

REGION_ZIP_CODES: dict[str, list[str]] = {
    "서울특별시": [
        "11110", "11140", "11170", "11200", "11215", "11230", "11260", "11290",
        "11305", "11320", "11350", "11380", "11410", "11440", "11470", "11500",
        "11530", "11545", "11560", "11590", "11620", "11650", "11680", "11710",
        "11740",
    ],
    "부산광역시": [
        "26110", "26140", "26170", "26200", "26230", "26260", "26290", "26320",
        "26350", "26380", "26410", "26440", "26470", "26500", "26530", "26710",
    ],
    "대구광역시": [
        "27110", "27140", "27170", "27200", "27230", "27260", "27290", "27710",
        "27720",
    ],
    "인천광역시": [
        "28125", "28177", "28185", "28200", "28237", "28245", "28290", "28710",
        "28720",
    ],
    "광주광역시": ["12110", "12130", "12150", "12170", "12190"],
    "대전광역시": ["30110", "30140", "30170", "30200", "30230"],
    "울산광역시": ["31110", "31140", "31170", "31200", "31710"],
    "세종특별자치시": ["36110"],
    "경기도": [
        "41111", "41113", "41115", "41117", "41131", "41133", "41135", "41150",
        "41171", "41173", "41192", "41194", "41196", "41210", "41220", "41250",
        "41271", "41273", "41281", "41285", "41287", "41290", "41310", "41360",
        "41370", "41390", "41410", "41430", "41450", "41461", "41463", "41465",
        "41480", "41500", "41550", "41570", "41591", "41593", "41595", "41597",
        "41610", "41630", "41650", "41670", "41800", "41820", "41830",
    ],
    "강원특별자치도": [
        "51110", "51130", "51150", "51170", "51190", "51210", "51230", "51720",
        "51730", "51750", "51760", "51770", "51780", "51790", "51800", "51810",
        "51820", "51830",
    ],
    "충청북도": [
        "43111", "43112", "43113", "43114", "43130", "43150", "43720", "43730",
        "43740", "43745", "43750", "43760", "43770", "43800",
    ],
    "충청남도": [
        "44131", "44133", "44150", "44180", "44200", "44210", "44230", "44250",
        "44270", "44710", "44760", "44770", "44790", "44800", "44810", "44825",
    ],
    "전북특별자치도": [
        "52111", "52113", "52130", "52140", "52180", "52190", "52210", "52710",
        "52720", "52730", "52740", "52750", "52770", "52790", "52800",
    ],
    "전라남도": [
        "12210", "12240", "12270", "12300", "12330", "12710", "12720", "12730",
        "12740", "12750", "12760", "12770", "12780", "12790", "12800", "12810",
        "12820", "12830", "12840", "12850", "12860", "12870",
    ],
    "경상북도": [
        "47111", "47113", "47130", "47150", "47170", "47190", "47210", "47230",
        "47250", "47280", "47290", "47730", "47750", "47760", "47770", "47820",
        "47830", "47840", "47850", "47900", "47920", "47930", "47940",
    ],
    "경상남도": [
        "48121", "48123", "48125", "48127", "48129", "48170", "48220", "48240",
        "48250", "48270", "48310", "48330", "48720", "48730", "48740", "48820",
        "48840", "48850", "48860", "48870", "48880", "48890",
    ],
    "제주특별자치도": ["50110", "50130"],
}


def zip_codes_for(region: str | None) -> list[str] | None:
    if not region:
        return None
    return REGION_ZIP_CODES.get(region)


__all__ = ["REGION_ZIP_CODES", "zip_codes_for"]
