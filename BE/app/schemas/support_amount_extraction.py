...
class SupportAmountExtraction(BaseModel):
    ...
    동일한 규칙을 따르되, 비율기반/케이스별 상이/애매한 경우는 확인필요로 통합한다.
    """

    determination: Literal["있음", "없음", "확인필요"] = Field(
        description=(
            "청년 개인(또는 청년 창업기업)에게 실제로 돈이 지급되는지 여부. "
            "대출/융자/보증 한도(상환해야 하는 돈), 사업 전체 예산/펀드 조성액, "
            "자격조건에 등장하는 숫자는 지원금액이 아니므로 '없음'으로 판정. "
            "비율기반 지원(소득구간별 매칭 적립, 이자 일부 지원 등)이거나 케이스별로 "
            "금액이 달라져 원문만으로 확정 지을 수 없으면 '확인필요'로 판정."
        )
    )
    ...
    amount_percent: Decimal | None = Field(
        default=None,
        description=(
            "determination이 '확인필요'이고 원문에 비율(%)이 명시된 경우 참고용 비율. "
            "숫자 하나만 넣을 것 — 원문이 '45~92.5%'처럼 범위면 상한값(92.5) 하나만."
        ),
    )
